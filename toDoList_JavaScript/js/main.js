let taskDOM = document.querySelector ("#task") //inuta girdiğimiz değer
let listDOM = document.querySelector ("#list") //listenin tamamı ul.
let allLiDOM = document.querySelectorAll("li") // quarySelector ile tüm li'lerden oluşan array'ı tanımlar. 

//liste elemanı silme fonksiyonu tanımladık. 
function removeElement(erase) {
    erase.remove();             //liste elemanını siler.
    eraseStroge.remove();       //Liste elemanının içeriğini LocalStorege'den siler.
}
